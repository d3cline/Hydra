        ...
        if len(start) == 13 and len(end) == 13:
            ...
            if not ioTank.objects.filter(owner=request.user):
                ...
                for i in readings:
                    ...
                    rtd_list.append(unixtime)
                    rtd_list.append(float(i.rtd))
                    rtd.append(rtd_list)

                    ec_list.append(unixtime)
                    ec_list.append(int(i.ec))
                    ec.append(ec_list)

                    do_list.append(unixtime)
                    do_list.append(int(i.do))
                    do.append(do_list)

                    ph_list.append(unixtime)
                    ph_list.append(float(i.ph))
                    ph.append(ph_list)

                    orp_list.append(unixtime)
                    orp_list.append(float(i.orp))
                    orp.append(orp_list)

            data = '{"data":{"t1":' + str(rtd) + ',"t2":' + str(ec) + ',"h":' + str(do) + ',"uv":' + str(
                ph) + ',"l":' + str(orp) + '}}'
    except ValueError:
...
# Calibrate Hydra
@login_required
def calibrate(request, uuid=None ):
    bot = ioTank.objects.get(id=uuid)
    return render(request, "calibration.html", {'bot': bot})
